...
```tsx #Definiowanie typów w języku TypeScript
let person: string
person = "Sam"
...
```tsx #Podstawowe typy i typy generyczne
...
```tsx #Ograniczenia typów generycznych
...
```tsx #Typy generyczne wewnątrz klasy
class Data {
...
```tsx #Typy generyczne, a union types
class Data<T extends string | number | boolean> {
  private data: T[] = []
