...
```ts #Definiowanie typów w języku TypeScript
let person: string
person = "Sam"
...
```ts #Podstawowe typy i typy generyczne
...
```ts #Ograniczenia typów generycznych
...
```ts #Typy generyczne wewnątrz klasy
class Data {
...
```ts #Typy generyczne, a union types
class Data<T extends string | number | boolean> {
  private data: T[] = []
