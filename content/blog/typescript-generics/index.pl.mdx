...
image:
  ...
  caption: 'Zdjęcie autorstwa Archana GS na Pexels'
...
Kontynuujmy naszą TypeScriptową serię. Dziś przyjrzymy się pojęciu, które jest dla mnie trochę niejasne — typy generyczne. Jeżeli znasz inne języki programowania takie jak C# lub Java, najprawdopodobniej zapoznałeś się z nimi. Istnieje jednak spora szansa, że podobnie do mnie, skupiasz się głównie na języku JavaScript (i być może jest to dla ciebie również niejasne). Nie ma takiego pojęcia w podstawowym języku, więc może powodować problemy. Jest to jednak niezbędne pojęcie języka TypeScript. Dlatego spróbuję wyjaśnić typy generyczne (tobie i sobie) jakbyśmy byli ośmiolatkami.
...
W czystym JavaScripcie możesz stworzyć obiekt, a następnie dodawać do niego właściwości. TypeScript nie lubi takiego dodawania „w locie”. Powiedzmy jednak, że potrzebujesz takiej funkcjonalności.
...
Partial owija typ i ustawia jego właściwości jako opcjonalne. W naszym przykładzie musimy ustalić `name` i `description` podczas tworzenia obiektu. Jednak, wykorzystując typ partial, nie musimy podawać obu wartości, gdy chcemy zaktualizować jeden z obiektów. Pola są opcjonalne i możemy je aktualizować niezależnie.
