...
description: 'Converting design tokens is an error-prone process—I found out the hard way. So, I made a simple Node.js script that will help me with that task.'
...
I have Node.js already installed on my machine. If you don't have this runtime, here's the [download link](https://nodejs.org/en/download/). You can also use [nvm](https://github.com/nvm-sh/nvm) to manage multiple versions of Node. For this project, I will use the latest version—18.3.0. You can check your Node version with `node -v` or `nvm ls`.

Having Node installed, let's init a new project with `npm init`. We probably won't use any third-party packages, but initiating a new one won't hurt. After getting through the npm setup, we should get a package.json file.
...
Having the object with styles imported, it's time for a more complex part—converting them into CSS styles. All tokens are in (nested) objects. For example, the body font size is sequentially nested in font, size, and body. We want to transform it into a CSS variable that looks like this: `--font-size-body: 1.5rem`. So, let's think about what we need to do. We need to concatenate keys from objects with hyphens, and when there are no more nested objects, we must add a string value to our freshly constructed CSS variable.
...
I refactored the final script a bit. I added some basic error handling with custom messages. Not only that, but I also extracted saving logic to an asynchronous function because the `writeFile()` method in our example is promise-based.
