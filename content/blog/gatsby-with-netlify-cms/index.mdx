...
![Wordpress Dahsboard](./wordpress-cms.png)
...
- `folder` or `files` - a collection type. The `folder` option takes a path to a folder. Inside this folder, there should be multiple files with the same format and fields (e.g., blog posts). The `files` option contains uniquely configured files (e.g., about page).
...
- `preview_path` - a template for generating paths to content on the live site. The tags are the same as the previous ones with exceptions:
