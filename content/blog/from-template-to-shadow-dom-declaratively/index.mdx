...
While DX is a matter of convenience, accessibility is not. ARIA references cross the shadow boundary in one direction only. Reference targeting, experimental as it is, forwards a reference aimed at the host to a single element inside—but an element in that shadow root still can't point at a label in the outer tree. Styling has its own gap: there is no declarative equivalent of `adoptedStyleSheets` or CSS modules, so shadow roots can't share a single stylesheet without JavaScript, though [declarative CSS module scripts](https://github.com/WICG/webcomponents/issues/939) are on the table. Despite the limitations, the future of web components looks promising.
