...
Shadow DOM is a second pillar of web components. To see the first, check my prior post—[Web components—what are custom elements?](/blog/intro-to-web-components-what-are-custom-elements/)
...
2. **Browser-internal shadow DOM.** Some built-in elements, like `<input>` or `<select>` use an internal shadow DOM created by a browser.
...
Why could we use it besides the cool name? Shadow DOM provides some key features for web components:
...
2. **Declaratively** in the HTML markup. The new API addition to the web platform.
...
Blocked JS queries don't mean that the `closed` mode is completely secure. Shadow DOM components still share the same JS execution context. If you need a security boundary for sensitive third-party script, use an `<iframe>`. Shadow DOM is about encapsulation—it's not a security measure.
...
| Non-inheritable properties (`background`, `border`, etc.) | No       | They are inherited neither in the shadow nor in the light DOM.               |
...
| `element.shadowRoot.querySelector()` | Depends  | It works only for `mode: "open"` shadow roots.                                    |
...
| `mouseenter`, `mouseleave`, `pointerenter`, `pointerleave` | No       | Non-bubbling and non-composed.                       |
...
| Custom events (`new CustomEvent(...)`)                     | Depends  | Only if `{ composed: true, bubbles: true }`.         |
...
- `slot.assign(node1, node2, ...)`: Manually assigns nodes to the slot. The shadow root must be created with the option `slotAssignment: "manual"` for this method to work. Throws error if called on an automatic slot.
...
The `:host` pseudo-class allows you to style custom element (shadow host).
...
There are two main ways to style web components (there are many twos in this post, shesh).
