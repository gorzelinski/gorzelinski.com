...
Blokowanie zapytań JS nie oznacza, że tryb `closed` jest całkowicie bezpieczny. Komponenty shadow DOM nadal dzielą ten sam kontekst wykonywania (ang. _execution context_). Jeżeli potrzebujesz bezpiecznej granicy dla zewnętrznego, niezaufanego skryptu, użyj `<iframe>`. Shadow DOM umożliwia enkapsulację — nie jest to środek bezpieczeństwa.
...
| `mouseenter`, `mouseleave`, `pointerenter`, `pointerleave` | Nie         | Skomponowane, ale niebąbelkujące—w praktyce nie przekraczają granicy.                                                     |
