...
- **Zmiana celu zdarzeń** (ang. _event retargetting_). Zdarzenia z shadow roota są dostępne od zewnątrz, ale ze zmienionym celem. Oznacza to, że kiedy zdarzenia wypływają z tego korzenia, ich cel jest zmieniony na element hosta, ukrywając wewnętrzny element, który autentycznie je wywołał.
...
- `delegateFocus`: Opcja ta pozwala kontrolować zachowanie focusa dla twoich komponentów. Przy ustawieniu `true`, kliknięcie gdziekolwiek na hosta lub używanie JavaScriptu do ustawienia na niego focusa, automatycznie ustawia focus na pierwszy dostępny element wewnątrz cienistego DOM-u.
- `slotAssignment`: Kontroluje w jaki sposób dzieci z jasnego DOM-u są przypisywane do elementów `<slot>`. Domyślny tryb `"named"` automatycznie dopasowuje dzieci do slotów bazując na ich atrybutach `slot="name"`. Tryb `"manual"` wyłącza to auto dopasowanie — zamiast tego, manualnie dopasowujesz elementy wykorzystując `slot.assign(element)`. Daje ci to kontrolę w rozmieszczaniu treści.
...
- `serializable`: Gdy ustawiona na `true`, shadow root może być serializowany do HTML-u wykorzystując `element.getHTML({ serializableShadowRoots: true })`. Łączy to światy deklaratywny i imperatywny — możesz stworzyć cienisty korzeń w JS-ie, a później wyeksportować go jako HTML zawierający `<template shadowrootmode>`.
...
Globalne selektory CSS nie przekraczają granicy w żadnym kierunku — twoja strona nie afektuje wnętrza shadow DOM-u, a style z wewnątrz nie wyciekają. Jest to duża zaleta używania cienistego DOM-u. Niedziedziczne właściwości CSS nie przekraczają granicy, podobnie jak nie są dziedziczone w jasnym DOM-ie. Jednakże dziedziczne właściwości CSS, jak `color` czy `font-family` przepływają naturalnie, bo cieniste elementy najwyższego poziomu dziedziczą po hoście.
...
| Niedziedziczne właściwości (`background`, `border`, etc.) | Nie         | Nie są dziedziczne w żadnym typie DOM-u (jasny)                  |
| Dziedziczne właściwości (`color`, `font-family`, etc.)    | Tak         | Cieniste elementy najwyższego poziomu dziedziczą po hoście.      |
...
| `element.shadowRoot.querySelector()` | Zależy      | Działa tylko dla shadow roota z `mode: "open"`.                                  |
...
| Most UI events (`click`, `keydown`, `input`, etc.)         | Tak         | Skomponowane domyślnie—bąbelkują przez granicę.   |
| Custom events (`new CustomEvent(...)`)                     | Zależy      | Tylko jeżeli `{ composed: true, bubbles: true }`. |
...
Niestety, największym problemem jest aktualnie dostępność. Jak widzieliśmy, ID mają ograniczony zasięg, uniemożliwiając odwołania przez granicę. Aktualnie jedynym rozwiązaniem jest właściwość `ariaLabelledByElements`. Jest to właściwość w języku JavaScript, która akceptuje odniesienia do elementów, omijając ten problem zasięgu. Jest wspierana przez wszystkie największe przeglądarki, ale nadal ma ograniczenia — element referencyjny musi być w tym samym lub nadrzędnym cienistym korzeniu i nie wspiera renderowania po stronie serwera. Także aktualnie nie ma prostego rozwiązania. Istnieje [propozycja celu referencyjnego](https://github.com/WICG/webcomponents/blob/gh-pages/proposals/reference-target-explainer.md), więc ekosystem zmierza w kierunku czystego rozwiązania.
...
Jest tu wiele do przyswojenia, więc nie powinieneś próbować zapamiętywać każdego detalu. Zapamiętaj, że ta cienista granica działa jak lustro weneckie. Dziedziczne style czy zmienne CSS wpływają do wewnątrz, umożliwiając stylowanie komponentów. Niektóre zdarzenia wypływają na zewnątrz, ale ich cel jest zmieniony, żeby ukryć wnętrzności. Standardowe zapytania DOM są całkowicie blokowane. Powoduje to problemy z dostępnością, ale na horyzoncie widnieją rozwiązania.
...
Jeżeli szukasz analogi, to sloty są podobne do `props.children` w świecie Reacta. Ponadto system slotów Vue był zainspirowany przez specyfikację slotów cienistego DOM-u. I podobnie, mamy tu dwa typy slotów:
...
- `element.assignedSlot`: Odwrotność — właściwość ta zwraca, do którego slotu przypisany jest dany element jasnego DOM-u.
- `slot.assign(node1, node2, ...)`: Manualnie przypisuje węzły do slota. Shadow root musi być stworzony z opcją `slotAssignment: "manual"`, aby ta metoda zadziałała. Wyrzuca błąd jeżeli jest wywoałana na automatycznym slocie.
...
Zdarzenie `slotchange` może zostać wywołane, podczas fazy parsowania HTML-a, przed [`connectedCallback`](/pl/blog/intro-to-web-components-what-are-custom-elements/#funkcje-cyklu-życia), więc musisz dodać odpowiednią logikę ochronną do swoich funkcji.
...
2. **Stylowanie od wewnątrz:** Jak dostarczasz API do konsumentów, aby mogli dostosować wygląd.
...
Inna pseudo klasa, `:host-context()`, pozwala aplikować style na podstawie tego, co jest w jasnym DOM-ie powyżej. W poniższym przykładzie ciemny kolor tła jest zaaplikowany tylko wtedy, gdy którykolwiek z potomków ma klasę ciemnego motywu.
...
Składnia `::part()` działa tylko do głębokości jednego poziomu. Aby stylować wewnętrzne komponenty tych zewnętrznych, skorzystaj z atrybutu `exportparts`. Celowo przekazuje on konkretne, wewnętrzne części, aby były dostępne z zewnątrz. `exportparts` działa podobnie do przekazywania propsów w React'cie.
