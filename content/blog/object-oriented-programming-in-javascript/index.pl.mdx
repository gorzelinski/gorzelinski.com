...
image:
  ...
  caption: 'Zdjęcie autorstwa Mayer Tawfik na Unsplash'
...
Z czasem ludzie zaczęli się zgadzać co do dobrych praktyk i języki obiektowe, takie jak C# czy Python, zostały wynalezione. Od tej pory programiści mogli modelować rzeczy z rzeczywistości przy pomocy obiektów. Obiekt to podstawowa, fundamentalna jednostka budulcowa wszystkich tych języków.
...
**Programowanie obiektowe** mówi, że umieszczanie danych i zachowań w jednym miejscu — obiekcie — upraszcza zrozumienie działania programu. Można to porównać to zapakowania wszystkiego w szczelnie zamkniętym pudełku.
...
To **nie** są zawody pod tytułem: „programowanie funkcjonalne kontra programowanie obiektowe”. Używałem OOP pisząc w Node i FP w React. Nie trzymaj się kurczowo jednego paradygmatu. Oba mają swoje zastosowania i mogą być komplementarne.
...
Możemy tworzyć odrębne obiekty dla każdej maszyny. Wyobraź sobie jednak, że chcemy teraz dziesięć kolejnych. Musielibyśmy ręcznie stworzyć obiekt dla każdego komputera albo kopiować wiele linii kodu. Stale byśmy się powtarzali.
...
Możemy tę funkcję wywołać wiele razy, aby stworzyć wiele obiektów. Nie powtarzamy się już tak bardzo. Istnieje jednak inny problem. Spójrz na funkcję `info()`. Pozostaje ona taka sama dla każdego komputera, a jednak kopiujemy ją za każdym razem. Nie jest to specjalnie oszczędne pod kątem pamięci.
...
Język JavaScript nie posiada klasycznego dziedziczenia znanego z języków takich jak Java czy C++. Posiada on tylko jedną konstrukcję — obiekt. To nie jest jednak bug — to jest feature! Każdy obiekt ma link do innego, który nazwany jest jego **prototypem.** Te powiązane obiekty tworzą **łańcuch prototypowy.**

</Callout>

Moglibyśmy się tu zatrzymać. Nie powtarzamy się i nasz kod nie zajmuje zbędnego miejsca. Możemy jednak także ulepszyć inne części paradygmatu OOP. Wcześniej wspominałem o tych zamkniętych pudełkach. Nie mamy tu tego i powyższy fragment kodu jest trochę trudny do zrozumienia. Użyjmy naszej wiedzy na temat prototypów i sprawdźmy, czy uda nam się coś ulepszyć.

### Funkcje konstruktora

Każda funkcja wywołana ze słowem `new` to funkcja konstruktora. Wbudowane obiekty takie jak `Array` czy `Function` są właśnie funkcjami konstrukcyjnymi. W języku JavaScript wszystko ma funkcję konstruktora (z wyjątkiem `null` i `undefined`). To słowo kluczowe `new` robi klika rzeczy za kulisami:
...
Uważaj jednak — to nie zadziała z funkcjami strzałkowymi. Funkcje strzałkowe używają zasięgu leksykalnego — definiują `this` na podstawie swojego miejsca w kodzie. Jeżeli zdefiniowalibyśmy funkcję `info()` jako strzałkową w naszym fragmencie, `this` wskazywałoby na obiekt `window`.

</Callout>

Programiści JavaScript pisali w ten sposób przez długi czas. Prawdopodobnie znalazłbyś podobne fragmenty kody w starszych repozytoriach. Wraz z ES6 dostaliśmy jednak nowy sposób, aby pisać kod obiektowy.
...
Uważaj — mimo że istnieje słowo `class`, to jest to tak zwany lukier składniowy. Pod spodem, cały czas używamy dziedziczenia prototypowego. W innych językach klasy są realną rzeczą. W przeciwieństwie do języka JavaScript, gdzie istnieją tylko obiekty.
...
Mówienie o filarach pod koniec wpisu może wydawać się dziwne. Są one jednak dobrym podsumowaniem i my już je poznaliśmy!
...
Nasze przykłady nie są skomplikowane, aby uprościć wywód. Jednak wyobraź sobie, że metoda `info()` jest złożona. Nie musisz wiedzieć, jak działa, aby jej użyć. Złożoność jest wyabstrahowana przed tobą. **Abstrakcja** oznacza, że ukrywamy złożoność przed użytkownikiem. Taki kod jest łatwy w użyciu.
...
Dobra, skłamałem. Nie poznaliśmy jeszcze jednego z filarów. Odkupuję swoje winy w tym momencie. Słowo „polimorfizm” pochodzi od greckiego słowa „polymorphos”, które dosłownie oznacza „wiele postaci”. Etymologia dobrze opisuje to, co chcemy osiągnąć polimorfizmem w OOP. Nie wiem, czy istnieje jedna, konkretna definicja polimorfizmu. Nie chcę kłócić się z ludźmi mądrzejszymi ode mnie, dlatego nie twierdzę, że przytaczam precyzyjną definicję. Pomysł jednak jest taki, że wywołujemy tę samą metodę na różnych obiektach i każdy z nich reaguje inaczej. JavaScript jest typowany dynamicznie i ogranicza polimorfizm. Jednakże możemy napisać coś takiego:
...
Odkryliśmy zasady, pojęcia i cele programowania obiektowego. Dodatkowo wiemy teraz jak je zastosować w przyszłych projektach JavaScript. Mam nadzieję, że ten post był pomocny i że udało ci się dotrwać do końca. A jeżeli nie masz dość, sprawdź także poniższe linki:
