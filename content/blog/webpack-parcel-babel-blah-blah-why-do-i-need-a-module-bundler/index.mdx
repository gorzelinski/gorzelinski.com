...
image:
  ...
  caption: 'Photo by Erda Estremera on Unsplash'
...
However, modern web apps are much more complex. They are highly interactive. They consist of multiple assets like images in different formats, videos, fonts, and third-party modules. Additionally, people use technologies like TypeScript, React, or Sass instead of those fundamental languages. Multiple files with various extensions import each other and different assets. But it is not available natively in the browser. Modern browsers have only just started to support module functionality natively. And you can't import assets inside a JS file. To resolve these problems, we need a tool.

## What is a module bundler?

A module bundler solves the mentioned problems. Fundamentally, the module bundler takes multiple JS files (modules) and combines them into a single file (bundle) that can build your app in a browser.
Apart from your local modules, it will bundle all third-party dependencies. Those dependencies can also have their own dependencies. A module bundler like Webpack will create a dependency graph to keep track of them. Of course, it's a simplification. To dive deeper and learn more features, let's configure a Webpack project.

<Callout variant="info">

With a CLI like `create-react-app` or frameworks like Gatsby, Webpack comes pre-configured. Still, I think it's good to know a little about what's under the hood.
...
Output customizes where Webpack will put our bundle. It takes an object with two parameters: `path` and `filename`. We'll customize Webpack to emit our bundle to `./public/main.js`.
...
If we configured everything correctly, Webpack should still work. But our bundle should contain much more code this time because we also bundle the React library. We can even check what creates our bundle.

### Plugins

Plugins are more powerful versions of loaders. You can use them to perform a broader range of tasks like bundle optimization or asset management. We want to analyze our bundle, and fortunately, there is a plugin we can use. First, let's install it.
...
Now, after running Webpack, a browser tab should open. You should see an interactive map with proportionally scaled rectangles. They symbolize project dependencies with additional pieces of information. In my opinion—pretty cool plugin.
