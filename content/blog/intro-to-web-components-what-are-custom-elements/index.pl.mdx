...
- `customElements.get()`: Zwraca konstruktor dla zarejestrowanego elementu lub zwraca `undefined`, jeżeli jest niezarejestrowany. Przydatna to wykrywania funkcjonalności lub sprawdzania, czy komponent jest dostępny.
...
2. I tu mamy twist — funkcja `attributeCahngeCallback()` następuje po konstruktorze. Jest tak, ponieważ parser ustawia atrybuty przed umieszczeniem elementu w drzewie DOM.
...
**Właściwości** żyją wewnątrz obiektów JS. W przeciwieństwie do atrybutów mogą być dowolnego typu: liczba, wartość logiczna, obiekt itd. _Nie wyzwalają_ metody `attributeChagnedCallback()`.
...
```js title="Nasłuchiwanie w constructor()"
class MyComponent extends HTMLElement {
  constructor() {
    super()
    this.addEventListener('click', this.handleClick)
