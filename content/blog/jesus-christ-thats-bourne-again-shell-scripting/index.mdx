...
The above test returns a exit code `0` if the directory exists. A **exit code (or exit status)** is a small integer that a program returns to the os when it finishes. Unlike other programming languages, Bash treats `0` as a success. It can also be treated as a boolean in conditionals:
