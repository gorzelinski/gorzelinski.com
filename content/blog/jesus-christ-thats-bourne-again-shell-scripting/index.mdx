...
Bash is a widely popular shell available on operating systems such as macOS or Linux. You can access it via a terminal and type commands inside. What's a shell? What's a terminal? What's a command? If you're unfamiliar with those terms, you can refer to the [first part](/blog/jesus-christ-thats-bourne-again-shell/) of this short series about Bash. In this one, we will focus on basic Bash syntax to write our first Bash scripts.
...
The `cd` command needs to be built-in because it operates on the shell's current directory. The `code` command opens the external program, Visual Studio Code. There are even commands that can be both, such as `echo`. It is a built-in command for efficiency, but it doesn't need to be. We can check that. To see which one you're using, you can type:

```bash
command -V echo
```

You should get a message that echo is a built-in command. The shell will run the built-in version by default. Then it will look for other programs. Those programs are located in the `/usr/bin/` directory.
...
We've been using them until now. These are commands presented in one line of text, often separated by a semicolon or piped (as in the example above). Be aware that “one line” doesn't mean a literal, singular horizontal line in the terminal. Lines can be very long and wrap in the terminal.
...
To make a script executable, you need to include a shebang as the first line (it should be pronounced “shuh·bang,” not “she bangs,” although [Ricky Martin](https://youtu.be/5ihtX86JzmA?si=V7MJAev-1EuDw6uT) would prefer the latter version). The line tells the operating system which interpreter to use to execute the file. As the sentence suggests, it doesn't need to be Bash. These files don't need an extension, but they need proper permissions. You can make a script executable using `chmod +x myscript`. Then you can run it directly by typing `./myscript` (or `myscript` if it's in your `$PATH`).
...
How do we declare constants if we should use lowercase? By using a specific flag.
...
A **test** in Bash is a command that evaluates a condition and returns an **exit status.** Square brackets wrap the condition. You can test various things, such as permissions, strings, numbers, and much more. To test if the home directory exists, we can type:
...
The above test returns an exit code `0` if the directory exists. An **exit code** (or exit status) is a small integer that a program returns to the OS when it finishes. Unlike other programming languages, Bash treats `0` as a success. It can also be treated as a boolean in conditionals:

- `0` equals `true` (yup, to confuse everyone).
- `1` (or any non-zero value) equals `false`. Different numbers often mean different failure reasons.
...
Passing arguments to scripts while invoking them is useful, but occasionally we need input while the script is running. For example, when you require multiple, consecutive answers from the user. In Bash, there are reserved keywords to gather input.
