...
image:
  ...
  caption: 'Zdjęcie autorstwa Chris Brignola na Unsplash'
...
W [ostatnim wpisie](/pl/blog/typescript-basic-types/) przedstawiłem przesłanki stojące za używaniem TypeScripta. Krótko go opisałem i wymieniłem podstawowe typy, przydatne podczas codziennej pracy z oprogramowaniem. Jednakże podstawowe typy to za mało, żeby budować współczesne, złożone aplikacje webowe.
...
Do tej pory mówiliśmy o metodach i właściwościach instancji. Istnieje jednak także sposób na dostęp do właściwości i metod bezpośrednio na klasie. Słowo kluczowe `static` definiuje statyczną metodę lub pole klasy. Działa zarówno w języku JavaScript jak i TypeScript. Najprawdopodobniej już miałeś okazję używać statycznych metod/właściwości. Wszystkie właściwości i metody obiektu `Math` — jak `Math.PI` lub `Math.abs()` — są statyczne.
