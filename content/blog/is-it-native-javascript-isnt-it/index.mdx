...
description: 'My first encounter with optional chaining and the nullish coalescing operator.'
...
I wondered, “What's with that question mark after the dot? Is it some special Gatsby syntax?” That didn't make sense to me, so I started looking for more information. After some research and Googling stupid questions, I found the term I was looking for—**optional chaining.** I was surprised—it's native JavaScript. This syntax was introduced in ES2020 (on the day of writing this post, it is at stage four of the proposal process). First, we look at some errors to see why it's useful.

<Callout variant="info">

**The 39nth Technical Committee (TC39)** is a group under ECMA International that contains JavaScript developers, implementers, and academics. The committee collaborates with the community to maintain and evolve the ECMAScript specification (JavaScript conforms to that specification). The development process has four stages. The fourth stage means the feature is ready to be included in the final draft of the specification.

</Callout>

If you're not a complete JavaScript beginner, you've probably seen a message like this: “Cannot read properties of undefined.” It usually means that you want to access a property of a nested object that doesn't exist. Let's reproduce this error, but this time on purpose. Imagine you intend to display detailed information about computers—laptops and desktops. They consist of almost the same parts, but some laptops don't have a dedicated graphics card—there is no information about it. If you try to access that information, you'll get our error.
...
Let's try to solve that error. Firstly, we can fill in that information.
...
But what if we couldn't? We don't always have control over the API. Another problem is when an object has more properties—each of them should be an empty `string`, `null`, etc. Not ideal. JavaScript is a dynamic programming language, so there can be `null` or `undefined` values. We need to deal with situations like this. We can use a logical expression to solve this.
...
const info = laptop
  ? laptop.graphicsCard
    ? laptop.graphicsCard.manufacturer
    : undefined
  : undefined //undefined
```
...
With optional chaining, you can try to access nested properties that may not be available. The above snippet won't throw an error. It will return `undefined`. If a reference is `null` or `undefined`, the expression returns `undefined`. In my opinion, the `?.` operator is concise and intuitive. It's like a question: “Is there a manufacturer (of the graphics card) property on the laptop object?” If so—return it. Otherwise, return `undefined`. This operator is more powerful. You can also use the syntax with function calls.
...
The object doesn't have any methods. Still, the snippet above doesn't throw an exception. It returns `undefined`. Let's go further—can you use this operator with arrays? Yes, you can.
...
Even though there are not four elements in the array, the code doesn't throw an error.
...
You can interpret the above snippet like this: “If a graphics card manufacturer in the laptop object exists, assign its value to the `info` variable. Otherwise, assign the integrated string.” From the previous section, we know that our optional chaining returns `undefined`. Then the interpreter moves to the logical operator. The nullish coalescing operator is a specific case of logical OR. It returns the right-hand side operand when its left-hand side operand is nullish (`null` or `undefined`). Logical OR returns the right-hand side operand if the left-hand side operand is any `falsy` value.

```js
const a = null ?? 'default' //"default"
const b = undefined ?? 'default' //"default"
const c = '' ?? 'default' //""
const d = NaN ?? 'default' //NaN - it returns the left-hand side for other falsy values
...
It is a shortcut for the expression with the nullish coalescing operator. The two expressions below are equivalent.
