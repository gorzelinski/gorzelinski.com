...
date: 2022-07-10T16:00:00+02:00
updated: 2022-07-10T16:00:00+02:00
image:
...
The object has not any method. Still, the snippet above doesn't throw an exception. It returns `undefined`. Let's go further - is it possible to use this operator with arrays? Yes, it is.
...
You can interpret the above snippet like this: "If there is a graphics card manufacturer in the laptop object, assign its value to info variable. Otherwise, assign the integrated string." From the previous section, we know that our optional chaining returns `undefined`. Then interpreter moves to the logical operator. The nullish coalescing operator is a special case of logical OR. It returns its right-hand side operand when its left-hand side operand is `null` or `undefined`. Logical OR returns the right-hand side operand if the left-hand side operand is any `falsy` value.
...
While researching this post, I found one more operator with a double question mark. If we "add" an equal sign to the nullish coalescing operator, we get **the logical nullish assignment.** It is easy to deduce what it is doing. Instead of returning, it assigns a particular value to x if x is nullish (null or undefined).
...
It is shortcut for the expression with the nullish coalescing operator. The two expressions below are equivalents.
