...
image:
  ...
  caption: 'Photo by Maxim Zhgulev on Unsplash'
...
Recently I've started learning TypeScript (technically, I used to code in Angular, so TypeScript is not new to me, but I haven't used it since then). I was hesitant at first, not going to lie. The TypeScript code seemed to me bloated and inelegant. But let's be honest—the only real downside of TypeScript is that you need to spend time learning it. I'm only human, after all—the change can be frightening. But change is the only constant in life. It can be good. And with the growing usage of TypeScript, this is a good moment to start learning it. Learn with me!

## What is TypeScript?

Fundamentally, TypeScript is a **superset** of JavaScript. What does this mean? Allow me to explain. You probably remember sets from school—you know, these intersecting circles. It's a Venn diagram—a graphical representation that shows the logical relation between these sets. There is a similar relationship between JavaScript and TypeScript—TypeScript is a superset of JavaScript, or JavaScript is a subset of TypeScript. Every line of JS code is valid TS code, but not the other way around.

![JavaScript and TypeScript venn diagram](js-ts-venn-diagram.png)

It means that TypeScript builds on JavaScript. It adds new features and advantages to the vanilla language. Actually, TypeScript is both **a language** and **a tool**. It's also a powerful compiler which you run to compile your TypeScript code to JavaScript. Why? Because browsers only understand JS code.
...
- Because of the compilation step, you can use modern JavaScript syntax without worrying about browser support. It offers similar advantages to using Babel.
...
Tuples are not available in vanilla JavaScript—they are added by TypeScript. A tuple is a fixed-length and typed array. It can be handy if you know the exact array structure beforehand.
...
Occasionally you want to accept more than one kind of value. That's where unions are helpful. You can use a pipe character between different types to define a union.
...
Some functions in JavaScript never return a value. “Wait, wait, how is that different from void?” We need to dive a little deeper into JavaScript to answer this question.
...
A function that has the `never` type never returns. It doesn't even return `undefined` implicitly. Such a situation occurs when a JavaScript function throws an error.
...
OK… I never (hehe) said I'll cover all types in one blog post. I'll end here. TypeScript offers even more primitive and advanced types, helping you produce more predictable and maintainable code. I'll cover more advanced concepts like [generics](/blog/typescript-generics/), [interfaces](/blog/object-oriented-programming-in-typescript/), or [decorators](/blog/typescript-decorators/) in the future. Meanwhile, you can check the following links.
