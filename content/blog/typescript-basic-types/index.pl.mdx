...
image:
  ...
  caption: 'Zdjęcie autorstwa Maxim Zhgulev na Unsplash'
...
Niedawno zacząłem uczyć się TypeScripta (w zasadzie kodowałem wcześniej w Angularze, więc TypeScript nie jest dla mnie zupełnie nowy, ale od tamtej pory nie używałem go często). Na początku miałem wątpliwości, nie będę kłamał. Kod w tym języku wydawał mi się opasły i nieelegancki. Bądźmy jednak szczerzy — jedynym minusem TypeScripta jest to, że musisz poświęcić czas na naukę. Jestem tylko człowiekiem, więc zmiana może być przerażająca. Jedyną jednak stałą życiu jest zmiana. Zmiany mogą być dobre. A wraz z rosnącym wykorzystaniem TypeScripta, to jest dobry moment, żeby zacząć się go uczyć. Ucz się ze mną!
...
Zacznijmy od tego, że TypeScript dostarcza dodatkowe typy do języka JavaScript. Istnieją typy, które są dostępne zarówno w JavaScripcie, jak i w TypeScripcie. Istnieją jednak też typy dostarczone przez nasz nadzbiór.
...
Wszystkie podstawowe typy w TypeScripcie są zapisane **małą literą.**
...
// W przypadku maksymalnego wieku dziewczyny DiCaprio to również jest stała - wiem.
...
Najbardziej podstawowy typ danych — reprezentuje wartość `true` lub `false`. W przeciwieństwie to JavaScriptu, TypeScript nie ma wartości „truthy” i „falsy”.
...
```typescript
// Oba parametry tej funkcji mogą być zarówno liczbą jak i ciągiem znaków.
...
```typescript
// Konwencja mówi, żeby zdefiniować niestandardowe typy z wielkiej litery.
