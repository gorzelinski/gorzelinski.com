...
description: "Let's learn how to decorate our cak… code with TypeScript decorators. Decorators are a tasty addition to classes and provide metaprogramming syntax."
...
Decorators are an experimental feature in TypeScript. TypeScript 5.0 introduced support for the new standard decorators, which can be attached to five different things: a class declaration, a method, an accessor, a property, and a parameter. So basically, they are all about the bass… class, I mean. You can use them for metaprogramming—a technique in which a program has knowledge of itself or can manipulate itself. They don't have a direct impact on the end user. It's a well-suited tool for writing code that is easier to use by other developers. Let's tune our brain instruments to learn some decorators!

<Callout variant="info">

At the time of writing, decorators are an upcoming [ECMAScript feature](https://devblogs.microsoft.com/typescript/announcing-typescript-5-0/#decorators). They are at stage 3 and may be added to native JavaScript. There is a paragraph describing the ECMAScript specification in one of my [previous posts](/blog/is-it-native-javascript-isnt-it/).

</Callout>

## Setup

Before learning decorators, we need a little setup work. To enable experimental support for decorators, you have two options.
...
Having the formalities out of the way, we can dive into decorators. A decorator, in the end, is just a standard JavaScript function—a function you apply to something. To apply a decorator, you need the at symbol (`@`). TypeScript uses this symbol to recognize a decorator.
...
Decorators usually use PascalCase (or UpperCamelCase)—you should capitalize the first letter.

</Callout>

## Decorator Factory

We can nest another function in our decorator, and this way, we get the decorator factory. The decorator factory returns a decorator function. We can configure a decorator before attaching it to something using this pattern. If you're getting a higher-order function vibe from this, your intuition is on point.
...
“How about attaching more than one decorator to something? Can we do that?” Yes, we can add more than one decorator to a class. Or anywhere decorators apply. The syntax is simple—stack decorators, one on top of another, like pancakes in the morning.
...
While we're touching on execution, let's grab the subject and check when decorators run. If you look into previous code snippets, you'll see that none of the above classes were instantiated. Nevertheless, you'll see the messages in the console. It's because all decorators (no matter the type) execute when you **define a class—not when you instantiate it.** They allow you to do additional, behind-the-scenes setup work when the class definition is evaluated.

## Types of Decorators

I mentioned different types of decorators. We have options when it comes to attaching decorators. Earlier, we saw class decorators. But we don't have to apply them directly to classes. We can add decorators, for example, to a property. Which arguments a decorator gets depends on where you add it.

A similar story is with return values. Some decorators are capable of returning something. What you can get back depends on which kind of decorator you work with. However, only in some decorators is the return value respected. Let's go through different decorator types.
...
The above example presents a recognizable `Component` decorator. This decorator marks a class as an Angular component. It accepts configuration and metadata that determine how this basic UI block should look and behave at runtime. You can pass there an HTML template, styling, or schemas.
...
Even though decorators don't influence end users, they improve the developer's experience. You can use them to watch property changes, transform parameters, validate at runtime, and many other things. They provide an elegant syntax to modify or extend the behavior of a class. Recently, TypeScript added this syntax as part of the language. Check my prior posts to learn more about OOP or TypeScript in general.
