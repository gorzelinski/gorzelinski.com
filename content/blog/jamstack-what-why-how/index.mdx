...
description: 'Websites and apps created with this new architecture are characterized by performance, security, and scalability.'
date: 2022-07-16T15:00:00+02:00
updated: 2024-08-24T11:20:00+02:00
image:
  alt: 'The row of jars with Jam labels'
  caption: 'Photo by Keji Gao on Unsplash'
  src: 'keji-gao-yHI9CyBZlfE-unsplash.jpg'
categories: ['web development']
tags: ['jamstack', 'gatsby', 'next', 'javascript', 'js', 'ssg']
type: 'post'
---

Jamstack is an alternative approach for creating websites and applications. It differs from traditional monolithic architectures.

![It's a comparison between traditional monolithic architecture and new JAMstack architecture. In this new approach, CDN and microservices replace different servers (app, web, etc.).](jamstack-architecture.png)

At the same time, it's not a bunch of random letters. People in the computer science field like acronyms. Jamstack is no exception. The following letters mean:
...
- **APIs**—interfaces that define interactions between software. Back-end operations—in this architecture—are abstracted to the APIs. You can reach them via HTTPS.
- **Markup**—a document in some format. Pages—in this approach—are served as HTML files. You can generate them with Markdown, MDX, or a static site generator.

Nevertheless, Jamstack is **not** a specific technology. This stack is not controlled (or defined) by any corporation. It is more like a movement and a community. Jamstack is a set of good practices and a working system. The system is convenient for developers and produces websites friendly for users.
...
Website performance matters. Web crawlers index websites, considering performance a significant factor. Users may leave if your website is loading slowly. I'm a patient guy (hey, I'm sitting here typing for another hour), but my patience is on the edge when I wait for a damn website to load. But I'm not alone. There was a study about [tolerable waiting time](https://www.tandfonline.com/doi/abs/10.1080/01449290410001669914). Most users are willing to wait for just about 2 seconds. And not all of them have fast internet connections.

The Jamstack approach precompiles pages. After a request, a user gets a finished page. You don't need to send many requests to different servers. A CDN sends your website with all the assets. The distributed nature of a CDN allows it to send them from the nearest edge. It shortens the route the files need to travel, and it saves time.
...
Simple architecture has one more benefit—improved security. Fewer elements mean fewer weak links. You don't have to worry about server or database breaches. Hackers have a smaller [attack surface](https://en.wikipedia.org/wiki/Attack_surface) to penetrate.

![You don't need to worry about a database if you don't use a database](roll-safe-meme.png)

### Developer's convenience

Simplifying some tasks has a positive effect on a developer's working comfort. On their mental health, probably too. You don't have to focus on complex configurations—you can focus on writing code. And a happy developer is a productive one. It may make the project better.
...
Content in a Jamstack project is usually stored in simple text files. It is in the same file structure as plugins, templates, and assets. Using Markdown or MDX, you can generate HTML pages that use custom components and templates (this blog post uses such a template). But creating content this way requires specific syntax. For non-technical users, there are convenient alternatives.

**Headless CMS** is a service that offers a rich user interface available via a browser. Using it, you can easily create and edit content. Headless means that only the data layer is editable. It's decoupled from the rendering layer (head), which you can choose. There may even be multiple rendering layers. Such a service exposes an API for downloading data in the build phase. A few examples are:
...
- [Netlify CMS](/blog/gatsby-with-netlify-cms/)
- Crafter CMS
- Forestry
...
- [Next.js](/blog/next-js-overview-in-1000-words/)
- Hugo
- Gatsby
- Jekyll
- Nuxt
...
Before making a website with this architecture, consider its pros and cons. This approach is not ideal for every application. If you've decided you want such a fast website or have questions, [write to me](/blog/jamstack-what-why-how/#contact).
