...
description: 'I enjoy reading books, and I spend many hours searching for them. So, I automated this process with a Python script that scrapes books from the web.'
...
I enjoy reading books. Different genres fill my bookshelf: economy, psychology, philosophy, biographies, nonfiction, and especially—science. The bonobo on Frans de Waal's book cover looks at Einstein's prominent face. Before I put anything there, I search. I can spend an hour before I click “buy” in an online bookstore. I look for recommendations everywhere: asking friends and strangers, browsing through Quora, Medium, and of course—Goodreads. Browsing the web is fun, but every hour spent searching takes time away from actual reading. So, like a real programmer, let's spend more hours automating this process!
...
Let's start with a plan. What do we want to achieve? We want a list of the best books for a particular subject or category. Goodreads has an enormous catalog of books, so we'll use it. Choosing “the best” books will be intersubjective; I'll use user ratings. It's not like the books on the list will be the best for everyone. But there is a high possibility we may like them. We will trust the wisdom of the crowd. Let's think about the steps we need to take.

1. Visit the Goodreads website.
2. Request a list of books for a particular subject.
3. Loop through multiple pages.
4. Process the response of every URL.
...
I'll be using Ubuntu, so I have Python preinstalled. If you use another OS, here's the [download link](https://www.python.org/downloads/). For this project, I'll be using Python 3.8.10. I'll also be using third-party Python modules. To avoid any trouble with versions, let's create a [virtual environment](https://packaging.python.org/en/latest/guides/installing-using-pip-and-virtual-environments/) (for Windows, the steps will differ a bit).
...
Now, we're ready to type sssome code. OK, I'm not going to use this joke again. Especially considering that the name comes from the comedy “Monty Python's Flying Circus” and not from a snake.
...
There's a lot going on here, so let's stop for a moment. For every book on a page, we want to extract info about the average rating and the number of ratings. First, we select the `span` with this information and grab text from it (the last element). We split the string using spaces and use list unpacking to take just the numbers. The average rating is first in the list, and the number of ratings is one before the last. We also need to convert the numbers to `float` and `int` because they are strings. I know I could use regex or iterate over the list to extract that info and not rely on positions, but I wanted to keep it simple. Then I also extracted the info about the title, author, and link.
...
Now, we can finally grab only the best books. Let's be picky—to be on our list, a title needs to have over 4.2 average and over 50,000 ratings. We can modify the variables `average_rating_threshold` and `ratings_threshold` to ease our conditions. Before the loop, I created the `best_books` list. Information about the book is added if both conditions are met.
...
I modified the URL to include the current page. I iterated over pages with the `range` function. After scraping books, the last step is left—saving them to a file. Below the scraping loop, I added another snippet that writes the best books to a Markdown file.
...
I thought running the script directly from the terminal without modifying it would be cool. So, I used custom command-line arguments to provide the necessary data. We can capture info passed to the script with the `argv` module. The first argument is the script name, and the others are custom.
...
Now we scrape books by typing in the terminal something like this:
