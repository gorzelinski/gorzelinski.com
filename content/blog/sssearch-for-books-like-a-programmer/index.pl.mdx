...
Wykorzystałem wbudowane funkcje języka Python: `open`, `write` i `close`, aby zapisać najlepsze książki do pliku. Pierwsza linia w pliku to nagłówek `h2` z informacją o kategorii. `itemgetter` to funkcja, która pozwala w elegancki sposób wydobyć dane ze słownika. Następnie użyłem tych danych, aby stworzyć element listy dla każdej książki. Znajduję się tam trochę znaczników HTML, aby wyglądało to ładniej. No i to wszystko - mamy najlepsze książki w pliku. Tu jest jego zawartość i to jak się wyświetla:
